...
Use the bundled Python helper for deterministic branch, default branch, existing PR, and diff-stat checks. Use the LLM only for judgement: deriving the concise PR title/body from the diff and asking when the branch prefix is unclear.
...
3. Use `branch_mapping` and `templates` to draft a PR title and body. Keep the title short and include the correct prefix (`[FEATURE]`, `[MAJOR]`, `[PATCH]`, or `[MINOR]`).
