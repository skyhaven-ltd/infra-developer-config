...
def inspect(t: str) -> dict[str,Any]:
    p=target_dir(t); remote=remote_origin(p); az=exe('az'); auth={'ok':False,'summary':'az not found'}; ext={'ok':False,'summary':'not checked'}
    if az:
        cp=run([az,'account','show']); auth={'ok':cp.returncode==0,'summary':'authenticated' if cp.returncode==0 else 'not authenticated'}
        ex=run([az,'extension','show','--name','azure-devops']); ext={'ok':ex.returncode==0,'summary':'installed' if ex.returncode==0 else 'missing'}
    return {'target':str(p),'origin':remote,'inferred_ado':parse_ado(remote),'az_path':az,'az_auth':auth,'azure_devops_extension':ext,'risk_flags':[k for k,b in {'az_not_found':not az,'az_not_authenticated':az and not auth['ok'],'azure_devops_extension_missing':az and not ext['ok'],'repository_not_inferred':not parse_ado(remote)}.items() if b], 'plan_shape': {'org':'org','project':'project','repo':'repo','rename_to':None,'enable_advanced_security':True,'approved':True}}

def apply(t, plan_path, dry):
    p=target_dir(t); plan=load_plan(plan_path); require_approved(plan); az=exe('az')
    if not az: raise SkillError('az executable not found')
    org=plan.get('org'); project=plan.get('project'); repo=plan.get('repo')
    if not all(isinstance(x,str) and x.strip() for x in [org,project,repo]): raise SkillError('org, project and repo are required')
    orgurl=org if org.startswith('https://') else f'https://dev.azure.com/{org}'
    commands=[['repos','update','--org',orgurl,'--project',project,'--repository',repo,'--default-branch','main']]
    if plan.get('rename_to'): commands.append(['repos','update','--org',orgurl,'--project',project,'--repository',repo,'--name',plan['rename_to']])
    if dry: return {'target':str(p),'dry_run':True,'would_run':commands}
    results=[]
    for c in commands:
        cp=run([az,*c]); results.append({'command':['az',*c],'returncode':cp.returncode,'stderr':cp.stderr.strip()[:500]})
    return {'target':str(p),'results':results}
